...
export const Elevation = (L.Control.Elevation = L.Control.extend({
  ...
  addTo(map) {
    if (this.options.detached) {
      ...
      parent
        ? _.append(parent, eleDiv)
        : _.insert(map.getContainer(), eleDiv, "afterend");
    } else {
  ...
  clear() {
    ...
    if (this._hotline)
      this._hotline.eachLayer((l) => l.options.renderer.remove()); // hotfix for: https://github.com/Raruto/leaflet-elevation/issues/233
  ...
  initialize(opts) {
    ...
    if (opts.followMarker)
      this._setMapView = _.throttle(this._setMapView, 300, this);
    ...
    if (opts.distanceMarkers === true)
      opts.distanceMarkers = Options.distanceMarkers;
  ...
  _addGeoJSONData(coords, properties, nestingLevel) {
    ...
    coords.forEach((point, i) => {
      ...
      point.prev = (attr) =>
        attr
          ? this._data[i > 0 ? i - 1 : 0][attr]
          : this._data[i > 0 ? i - 1 : 0];

      this.fire("elepoint_init", {
        point: point,
        props: props,
        id: i,
        isMulti: nestingLevel,
      });
      ...
      if (
        this._yCoordMax < this._data[this._data.length - 1][this.options.yAttr]
      )
        this._yCoordMax = this._data[this._data.length - 1][this.options.yAttr];
    });
  ...
  _addLayer(layer) {
    ...
    if (
      layer &&
      this.options.distanceMarkers &&
      this.options.distanceMarkers.lazy
    ) {
      layer.on(
        "add remove",
        ({ target, type }) =>
  ...
  _initLayer() {
    this._layers.on("layeradd layerremove", ({ layer, type }) => {
   ...
   */
  _loadLayer(geojson) {
    let { polyline, theme, waypoints, wptIcons, wptLabels, distanceMarkers } =
      this.options;
  ...
  _onToggleChart({ name, enabled }) {
    ...
    this.options[name] =
      !enabled && this.options[name] == "disabled" ? "enabled" : "disabled";
  ...
  _parseFromXMLString(data) {
    ...
    if (!(type in toGeoJSON)) {
      type =
        xml.documentElement.tagName == "TrainingCenterDatabase" ? "tcx" : "gpx";
    }
    ...
    this.on("elepoint_added", ({ index, point }) => {
      ...
      if (props.stats) {
        for (const key in props.stats) {
          ...
          this.track_info[sname + key] = props.stats[key].call(
            this,
            curr[attr],
      ...
      if (props.onPointAdded)
        props.onPointAdded.call(this, curr[attr], i, point);
    });
  },
  ...
  _registerHandler(props) {
    ...
    if (this.options[name] || required) {
      ...
      if (this.options[name] !== "summary") {
        if (scale)
          this._registerAxisScale(L.extend({ name, label: unit }, scale));
  ...
  _registerMarker({ latlng, sym, content }) {
    ...
    if (content) {
      marker
        .bindPopup(content, { className: "elevation-popup", keepInView: true })
        .openPopup();
      marker
        .bindTooltip(content, {
  ...
  _resizeChart() {
    if (this._container && _.style(this._container, "display") != "none") {
      let opts = this.options;
      let newWidth = opts.detached
        ? (this.eleDiv || this._container).offsetWidth
        : _.clamp(opts._maxWidth, [
            0,
            this._map.getContainer().clientWidth - 30,
          ]);
      if (newWidth) {
        opts.width = newWidth;
   ...
   */
  _toggle() {
    _.hasClass(this._container, "elevation-expanded")
      ? this._collapse()
      : this._expand();
  },
  ...
  _updateSummary() {
    if (this._summary) {
      ...
      if (this.options.downloadLink && this._downloadURL) {
        ...
        this._summary._container.innerHTML +=
        ...
        _.select(".download a", this._summary._container).onclick = (e) => {
          ...
          if (
            this.options.downloadLink == "modal" &&
            ...
            this.options.downloadLink === true
          ) {
            event.confirm();
          }
