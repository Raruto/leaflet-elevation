...
export function resolveURL(src, baseUrl) {
  return new URL(
    src,
    src.startsWith("../") || src.startsWith("./") ? baseUrl : undefined,
  ).toString();
}
...
export function formatDate(format) {
  if (!format) {
    return (time) =>
      new Date(time).toLocaleString().replaceAll("/", "-").replaceAll(",", " ");
  } else if (format == "time") {
...
export function coordPropsToMeta(coordProps, name, parser) {
  return (
    coordProps &&
    (({ props, point, id, isMulti }) => {
      if (props) {
        for (const key of coordProps) {
          if (key in props) {
            point.meta[name] = (parser || parseNumeric).call(
              this,
              isMulti ? props[key][isMulti] : props[key],
              id,
            );
            break;
          }
 ...
 */
export const parseNumeric = (property, id) =>
  parseInt(typeof property === "object" ? property[id] : property);

/**
 * Extract datetime property (id) from GeoJSON object
 */
export const parseDate = (property, id) =>
  new Date(Date.parse(typeof property === "object" ? property[id] : property));

/**
 * A little bit shorter than L.DomUtil
 */
export const addClass = (n, str) =>
  n && str.split(" ").every((s) => s && L.DomUtil.addClass(n, s));
export const removeClass = (n, str) =>
  n && str.split(" ").every((s) => s && L.DomUtil.removeClass(n, s));
export const toggleClass = (n, str, cond) =>
  (cond ? addClass : removeClass)(n, str);
export const replaceClass = (n, rem, add) =>
  (rem && removeClass(n, rem)) || (add && addClass(n, add));
export const style = (n, k, v) =>
  (typeof v === "undefined" && L.DomUtil.getStyle(n, k)) ||
  n.style.setProperty(k, v);
export const toggleStyle = (n, k, v, cond) => style(n, k, cond ? v : "");
export const setAttributes = (n, attrs) => {
  for (let k in attrs) {
    n.setAttribute(k, attrs[k]);
  }
};
export const toggleEvent = (el, e, fn, cond) => el[cond ? "on" : "off"](e, fn);
export const create = (tag, str, attrs, n) => {
  let elem = L.DomUtil.create(tag, str || "");
 ...
 */
export const clamp = (val, range) =>
  range ? (val < range[0] ? range[0] : val > range[1] ? range[1] : val) : val;

/**
 * Limit a delta difference between two values
 */
export const wrapDelta = (curr, prev, deltaMax) =>
  Math.abs(curr - prev) > deltaMax
    ? prev + deltaMax * Math.sign(curr - prev)
    : curr;
        ...
        Object.defineProperty(
          ...
          propdesc.get || propdesc.set
            ...
            : {
                ...
                value: skipProps.includes(prop)
                  ? propdesc.value
                  : cloneDeep(propdesc.value, skipProps, cache),
              },
        );
      });
      return copy;
