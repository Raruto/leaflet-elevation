export function Pace() {
  ...
  return {
    ...
    scale: this.options.pace &&
      this.options.pace != "summary" && {
        axis: "y",
        position: "right",
        scale: { min: 0, max: +1 },
        tickPadding: 16,
    ...
    summary: this.options.pace && {
      minpace: {
        label: "Min Pace: ",
        value: (track, unit) =>
          Math.round(track.pace_min || 0) + "&nbsp;" + unit,
        order: 51,
      },
      maxpace: {
        label: "Max Pace: ",
        value: (track, unit) =>
          Math.round(track.pace_max || 0) + "&nbsp;" + unit,
        order: 51,
      },
      avgpace: {
        label: "Avg Pace: ",
        value: (track, unit) =>
          Math.round(track.pace_avg || 0) + "&nbsp;" + unit,
        order: 52,
      },
    },
  };
