...
export function Cadence() {
  return {
    ...
    pointToAttr: (point, i) =>
      (point.cad ?? point.meta.cad ?? point.prev("cadence")) || 0,
    ...
    summary: {
      minrpm: {
        label: "Min RPM: ",
        value: (track, unit) =>
          Math.round(track.cadence_min || 0) + "&nbsp;" + unit,
        // order: 30
      },
      maxrpm: {
        label: "Max RPM: ",
        value: (track, unit) =>
          Math.round(track.cadence_max || 0) + "&nbsp;" + unit,
        // order: 30
      },
      avgrpm: {
        label: "Avg RPM: ",
        value: (track, unit) =>
          Math.round(track.cadence_avg || 0) + "&nbsp;" + unit,
        // order: 20
      },
    },
  };
