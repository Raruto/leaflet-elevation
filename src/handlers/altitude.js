...
export function Altitude() {
  ...
  opts.altitudeFactor = opts.imperial
    ? this.__footFactor
    : opts.altitudeFactor || 1; // 1 m = (1 m)
  ...
  return {
    ...
    tooltip: {
      ...
      chart: (item) =>
        L._("y: ") +
        _.round(item[opts.yAttr], opts.decimalsY) +
        " " +
        altitude.label,
      marker: (item) =>
        _.round(item[opts.yAttr], opts.decimalsY) + " " + altitude.label,
      order: 10,
    },
    summary: {
      minele: {
        label: "Min Elevation: ",
        value: (track, unit) =>
          (track.elevation_min || 0).toFixed(2) + "&nbsp;" + unit,
        order: 30,
      },
      maxele: {
        label: "Max Elevation: ",
        value: (track, unit) =>
          (track.elevation_max || 0).toFixed(2) + "&nbsp;" + unit,
        order: 31,
      },
      avgele: {
        label: "Avg Elevation: ",
        value: (track, unit) =>
          (track.elevation_avg || 0).toFixed(2) + "&nbsp;" + unit,
        order: 32,
      },
    },
  };
