...
export function Speed() {
  ...
  return {
    ...
    pointToAttr: (_, i) => {
      ...
      return dt > 0
        ? Math.abs((dx / dt) * opts.timeFactor) * opts.speedFactor
        : NaN;
    },
    ...
    summary: this.options.speed && {
      minspeed: {
        label: "Min Speed: ",
        value: (track, unit) =>
          Math.round(track.speed_min || 0) + "&nbsp;" + unit,
        order: 51,
      },
      maxspeed: {
        label: "Max Speed: ",
        value: (track, unit) =>
          Math.round(track.speed_max || 0) + "&nbsp;" + unit,
        order: 51,
      },
      avgspeed: {
        label: "Avg Speed: ",
        value: (track, unit) =>
          Math.round(track.speed_avg || 0) + "&nbsp;" + unit,
        order: 52,
      },
    },
  };
