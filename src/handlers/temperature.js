export function Temperature() {
  ...
  opts.temperatureFactor1 =
    opts.temperatureFactor1 ?? (opts.imperial ? 1.8 : 1);
  ...
  return {
    ...
    decimals: 2,
    pointToAttr: (point, i) =>
      (point.meta.atemps ?? point.meta.atemps ?? point.prev("temperature")) *
        opts.temperatureFactor1 +
      opts.temperatureFactor2,
    ...
    tooltip: {
      ...
      chart: (item) =>
        L._("Temp: ") +
        Math.round(item.temperature).toLocaleString() +
        " " +
        temperature.label,
      marker: (item) =>
        Math.round(item.temperature).toLocaleString() + " " + temperature.label,
      order: 1,
    },
    summary: {
      mintemp: {
        label: "Min Temp: ",
        value: (track, unit) =>
          Math.round(track.temperature_min || 0) + "&nbsp;" + unit,
      },
      maxtemp: {
        label: "Max Temp: ",
        value: (track, unit) =>
          Math.round(track.temperature_max || 0) + "&nbsp;" + unit,
      },
      avgtemp: {
        label: "Avg Temp: ",
        value: (track, unit) =>
          Math.round(track.temperature_avg || 0) + "&nbsp;" + unit,
      },
    },
  };
}
