...
export const Axis = ({
  ...
  return (g) => {
    ...
    let axisScale = d3[
      "axis" + position.replace(/\b\w/g, (l) => l.toUpperCase())
    ]()
      .scale(scale)
...
export const PositionMarker = ({
  ...
  return (g) => {
    ...
    text
      .select("tspan")
      .attr("dy", text.selectAll("tspan").size() > 1 ? "-1.5em" : "0em");
...
export const LegendItem = ({
  ...
  return (g) => {
    ...
    g.on("click.legend", () =>
      d3.select(g.node().ownerSVGElement || g).dispatch("legend_clicked", {
        detail: {
          path: path.node(),
          name: name,
          legend: g.node(),
...
export const LegendSmall = ({ width, height, items, onClick }) => {
  return (g) => {
    let idx = 0;
...
export const Tooltip = ({
  ...
  return (g) => {
    ...
    if (yCoord)
      yAlign = Math.max(yCoord - bbox.height, L.Browser.webkit ? 0 : -Infinity);
...
export const Ruler = ({ height, width }) => {
  return (g) => {
    g.data([
      {
        x: 0,
    ...
    let symbol = g.selectAll(".horizontal-drag-symbol").data([
      {
...
export const CheckPoint = ({ point, height, width, x, y }) => {
  return (g) => {
...
export const Domain = ({ min, max, attr, name, forceBounds, scale }) =>
  function (data) {
...
export const Range = ({ axis }) =>
  function (width, height) {
    if (axis == "x") return [0, width];
    else if (axis == "y") return [height, 0];
  };

export const Scale = ({ data, attr, min, max, forceBounds, range }) => {
  return d3
    .scaleLinear()
    .range(range)
...
export const Bisect = ({ data = [0, 1], scale, x, attr }) => {
  return d3.bisector((d) => d[attr]).left(data, scale.invert(x));
};

export const Chart = ({ width, height, margins = {}, ruler }) => {
  ...
  // SVG Paths
  const mask = panes.area
    .append("svg:mask")
    .attr("id", "elevation-clipper-" + Math.random().toString(36).substr(2, 9))
    .attr("fill-opacity", 1);
  const maskRect = mask
    .append("svg:rect")
    .attr("class", "zoom")
    .attr("fill", "white"); // white = transparent

  // Canvas Paths
  const foreignObject = panes.area
    .append("svg:foreignObject")
    .attr("mask", "url(#" + mask.attr("id") + ")");
  const canvas = foreignObject
    .append("xhtml:canvas")
  ...
  // Resize
  chart._resize = ({ width, height, margins = {}, ruler }) => {
    const w = width - margins.left - margins.right;
    const h = height - margins.top - margins.bottom;
  ...
  chart.pane = (name) =>
    panes[name] || (panes[name] = g.append("g").attr("class", name));
