...
export var Chart = (L.Control.Elevation.Chart = L.Class.extend({
  ...
  _initClipper() {
    ...
    const onZoom = ({ transform, sourceEvent }) => {
      ...
      if (this._scales.time)
        this._scales.time = transform.rescaleX(this._scales.time); // calculate x scale at zoom level
      this._resetDrag();
  ...
  _initRuler() {
    ...
    const label = (e, d) => {
      ...
      if (y >= yMax || y <= 0)
        this._ruler.select(".horizontal-drag-label").text("");
      this._hideDiagramIndicator();
    };
  ...
  _registerAreaPath(props) {
    ...
    if (typeof props.preferCanvas === "undefined")
      props.preferCanvas = opts.preferCanvas;
  ...
  _updateAxis() {
    ...
    this._axis.selectAll(".y.axis.right").each((d, i, n) => {
  ...
  _findCoordsForY(y) {
    ...
    const coords = list.reduce((array, _, curr) => {
      ...
      if (list[next] !== list[curr] + 1 || next === list.length) {
        array.push(list.slice(start, next).map((i) => data[i].latlng));
        start = next;
      }
      return array;
    }, []);
  ...
  _showDiagramIndicator(item, xCoordinate) {
    this._tooltip.attr("display", null).call(
      D3.Tooltip({
        xCoord: xCoordinate,
  ...
  _togglePath(name, enabled = true, lazy = false) {
    ...
    legend
      .select("text")
      .style("text-decoration-line", enabled ? "" : "line-through");
