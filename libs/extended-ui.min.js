...
min:-1,max:+1,forceBounds:opts.forceAxisBounds});slope.axis=D3.Axis({axis:"y",position:"right",width:this._width(),height:this._height(),scale:slope.y,ticks:this.options.yTicks,tickPadding:16,label:"%",labelX:25,labelY:3});this._axis.call(slope.axis)});this.on("elechart_updated",function(){slope.area=D3.Area({interpolation:"curveStepAfter",data:this._data,name:"Slope",xAttr:opts.xAttr,yAttr:"slope",width:this._width(),height:this._height(),scaleX:slope.x,scaleY:slope.y});slope.path.call(slope.area)});
