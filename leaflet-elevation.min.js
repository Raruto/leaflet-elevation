...
var prev=data.length?data[data.length-1].latlng:curr;var delta=curr.distanceTo(prev)*this._distanceFactor;dist=dist+Math.round(delta/1E3*1E5)/1E5;if(!this.options.skipNullZCoords&&data.length>0){var prevZ=data[data.length-1].z;if(isNaN(prevZ)){var lastZ=this._lastValidZ;var currZ=z*this._heightFactor;if(!isNaN(lastZ)&&!isNaN(currZ))prevZ=(lastZ+currZ)/2;else if(!isNaN(lastZ))prevZ=lastZ;else if(!isNaN(currZ))prevZ=currZ;data[data.length-1].z=prevZ}}if(!isNaN(z)){z=z*this._heightFactor;eleMax=eleMax<
z?z:eleMax;eleMin=eleMin>z?z:eleMin;data.push({dist:dist,x:x,y:y,z:z,latlng:curr});this._lastValidZ=z}this._data=data;this._distance=dist;this._maxElevation=eleMax;this._minElevation=eleMin},_addToChartDiv:function(map){this._appendElevationDiv(map._container).appendChild(this.onAdd(map))},_appendChart:function(svg){var g=svg.append("g").attr("transform","translate("+this.options.margins.left+","+this.options.margins.top+")");this._appendGrid(g);this._appendAreaPath(g);this._appendAxis(g);this._appendFocusRect(g);
this._appendMouseFocusG(g);this._appendLegend(g)},_appendElevationDiv:function(container){var eleDiv=document.querySelector(this.options.elevationDiv);if(!eleDiv){eleDiv=L.DomUtil.create("div","leaflet-control elevation elevation-div");this.options.elevationDiv="#elevation-div_"+Math.random().toString(36).substr(2,9);eleDiv.id=this.options.elevationDiv.substr(1);container.parentNode.insertBefore(eleDiv,container.nextSibling)}if(this.options.detached){L.DomUtil.addClass(eleDiv,"elevation-detached");
L.DomUtil.removeClass(eleDiv,"leaflet-control")}this.eleDiv=eleDiv;return this.eleDiv},_appendXaxis:function(axis){axis.append("g").attr("class","x axis").attr("transform","translate(0,"+this._height()+")").call(d3.axisBottom().scale(this._x).ticks(this.options.xTicks)).append("text").attr("x",this._width()+6).attr("y",30).text(this._xLabel)},_appendXGrid:function(grid){grid.append("g").attr("class","x grid").attr("transform","translate(0,"+this._height()+")").call(d3.axisBottom().scale(this._x).ticks(this.options.xTicks).tickSize(-this._height()).tickFormat(""))},
...
this._focuslabelY=this._focuslabeltext.append("svg:tspan").attr("class","mouse-focus-label-y").attr("dy","-1em");this._focuslabelX=this._focuslabeltext.append("svg:tspan").attr("class","mouse-focus-label-x").attr("dy","2em")},_appendLegend:function(g){var legend=this._legend=g.append("g").attr("class","legend");var altitude=this._altitudeLegend=this._legend.append("g").attr("class","legend-altitude");altitude.append("rect").attr("class","area").attr("x",this._width()/2-50).attr("y",this._height()+
this.options.margins.bottom-17).attr("width",50).attr("height",5).attr("opacity",.75);altitude.append("text").text("Altitude").attr("x",this._width()/2+5).attr("font-size",10).style("text-decoration-thickness","2px").style("font-weight","700").attr("y",this._height()+this.options.margins.bottom-11)},_appendPositionMarker:function(pane){var theme=this.options.theme;var heightG=pane.select("g");this._mouseHeightFocus=heightG.append("svg:line").attr("class",theme+" height-focus line").attr("x2",0).attr("y2",
0).attr("x1",0).attr("y1",0);this._pointG=heightG.append("g");this._pointG.append("svg:circle").attr("class",theme+" height-focus circle-lower").attr("r",6).attr("cx",0).attr("cy",0);this._mouseHeightFocusLabel=heightG.append("svg:text").attr("class",theme+" height-focus-label").style("pointer-events","none")},_applyData:function(){if(!this._data)return;var xdomain=d3.extent(this._data,function(d){return d.dist});var ydomain=d3.extent(this._data,function(d){return d.z});var opts=this.options;if(opts.yAxisMin!==
undefined&&(opts.yAxisMin<ydomain[0]||opts.forceAxisBounds))ydomain[0]=opts.yAxisMin;if(opts.yAxisMax!==undefined&&(opts.yAxisMax>ydomain[1]||opts.forceAxisBounds))ydomain[1]=opts.yAxisMax;this._x.domain(xdomain);this._y.domain(ydomain);this._areapath.datum(this._data).attr("d",this._area);this._updateAxis();this._fullExtent=this._calculateFullExtent(this._data)},_calculateFullExtent:function(data){if(!data||data.length<1)throw new Error("no data in parameters");var ext=new L.latLngBounds(data[0].latlng,
