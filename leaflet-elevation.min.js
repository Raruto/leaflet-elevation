...
a.target="_new";a.download="";a.style.display="none";b.appendChild(a);a.click();b.removeChild(a)},_dragHandler:function(){d3.event.preventDefault();d3.event.stopPropagation();this._gotDragged=true;this._drawDragRectangle()},_dragEndHandler:function(){if(!this._dragStartCoords||!this._dragCurrentCoords||!this._gotDragged){this._dragStartCoords=null;this._gotDragged=false;if(this._draggingEnabled)this._resetDrag();if(this.options.legend&&this._chartEnabled){this._clearChart();this._clearPath();this._chartEnabled=
false}else{this._resizeChart();this._chartEnabled=true}return}var item1=this._findItemForX(this._dragStartCoords[0]),item2=this._findItemForX(this._dragCurrentCoords[0]);if(item1==item2)return;this._hidePositionMarker();this._fitSection(item1,item2);this._dragStartCoords=null;this._gotDragged=false;var evt={data:{dragstart:this._data[item1],dragend:this._data[item2]}};if(this.fire)this.fire("elechart_dragged",evt,true);if(this._map)this._map.fire("elechart_dragged",evt,true)},_dragStartHandler:function(){d3.event.preventDefault();
