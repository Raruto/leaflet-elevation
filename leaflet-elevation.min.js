...
iconSize:[30,30],iconAnchor:[8,30]})}},polyline_options:{className:"",color:"#566B13",opacity:.75,weight:5,lineCap:"round"}},height:200,heightFactor:1,hoverNumber:{decimalsX:2,decimalsY:0,formatter:undefined},imperial:false,interpolation:"curveLinear",lazyLoadJS:true,loadData:{defer:false,lazy:false},marker:"elevation-line",markerIcon:L.divIcon({className:"elevation-position-marker",html:'<i class="elevation-position-icon"></i>',iconSize:[32,32],iconAnchor:[16,16]}),placeholder:false,position:"topright",
reverseCoords:false,theme:"lightblue-theme",margins:{top:10,right:20,bottom:30,left:50},responsive:true,summary:"inline",width:600,xLabel:"km",xTicks:undefined,yAxisMax:undefined,yAxisMin:undefined,yLabel:"m",yTicks:undefined,zFollow:13},__mileFactor:.621371,__footFactor:3.28084,addData:function(d,layer){this._addData(d);if(this._container)this._applyData();if((typeof layer==="undefined"||layer===null)&&d.on)layer=d;if(layer){if(layer._path)L.DomUtil.addClass(layer._path,"elevation-polyline "+this.options.theme);
layer.on("mousemove",this._mousemoveLayerHandler,this).on("mouseout",this._mouseoutHandler,this)}this.track_info=this.track_info||{};this.track_info.distance=this._distance;this.track_info.elevation_max=this._maxElevation;this.track_info.elevation_min=this._minElevation;this._layers=this._layers||{};this._layers[L.Util.stamp(layer)]=layer;var evt={data:d,layer:layer,track_info:this.track_info};if(this.fire)this.fire("eledata_added",evt,true);if(this._map)this._map.fire("eledata_added",evt,true)},
...
L.bind(function(data){if(!ticking){L.Util.requestAnimFrame(function(){if(this._isVisible(this.placeholder)){window.removeEventListener("scroll",scrollFn);this.loadData(data,opts);this.once("eledata_loaded",function(){if(this.placeholder)this.placeholder.parentNode.removeChild(this.placeholder)},this)}ticking=false},this);ticking=true}},this,data);window.addEventListener("scroll",scrollFn);scrollFn()},onAdd:function(map){this._map=map;var container=this._container=L.DomUtil.create("div","elevation-control elevation");
if(!this.options.detached)L.DomUtil.addClass(container,"leaflet-control");if(this.options.theme)L.DomUtil.addClass(container,this.options.theme);if(this.options.placeholder&&!this._data){this.placeholder=L.DomUtil.create("img","elevation-placeholder");if(typeof this.options.placeholder==="string"){this.placeholder.src=this.options.placeholder;this.placeholder.alt=""}else for(var i in this.options.placeholder)this.placeholder.setAttribute(i,this.options.placeholder[i]);container.insertBefore(this.placeholder,
container.firstChild)}var callback=function(map,container){this._initToggle(container);this._initChart(container);this._applyData();this._map.on("zoom viewreset zoomanim",this._hidePositionMarker,this);this._map.on("resize",this._resetView,this);this._map.on("resize",this._resizeChart,this);this._map.on("mousedown",this._resetDrag,this);this._map.on("eledata_loaded",this._updateSummary,this);L.DomEvent.on(this._map._container,"mousewheel",this._resetDrag,this);L.DomEvent.on(this._map._container,"touchstart",
this._resetDrag,this)}.bind(this,map,container);if(typeof d3!=="object"&&this.options.lazyLoadJS){L.Control.Elevation._d3LazyLoader=this._lazyLoadJS("https://unpkg.com/d3@4.13.0/build/d3.min.js",L.Control.Elevation._d3LazyLoader);L.Control.Elevation._d3LazyLoader.then(callback)}else callback.call();return container},onRemove:function(map){this._container=null},setZFollow:function(zoom){this._zFollow=zoom},show:function(){this._container.style.display="block"},_addData:function(d){var geom=d&&d.geometry&&
d.geometry;var i;if(geom)switch(geom.type){case "LineString":this._addGeoJSONData(geom.coordinates);break;case "MultiLineString":for(i=0;i<geom.coordinates.length;i++)this._addGeoJSONData(geom.coordinates[i]);break;default:console.warn("Unsopperted GeoJSON feature geometry type:"+geom.type)}var feat=d&&d.type==="FeatureCollection";if(feat)for(i=0;i<d.features.length;i++)this._addData(d.features[i]);if(d&&d._latlngs)this._addGPXdata(d._latlngs)},_addGeoJSONData:function(coords){if(coords)for(var i=
0;i<coords.length;i++)this._addPoint(coords[i][1],coords[i][0],coords[i][2])},_addGPXdata:function(coords){if(coords)for(var i=0;i<coords.length;i++)this._addPoint(coords[i].lat,coords[i].lng,coords[i].meta.ele)},_addPoint:function(x,y,z){if(this.options.reverseCoords){var tmp=x;x=y;y=tmp}var data=this._data||[];var eleMax=this._maxElevation||-Infinity;var eleMin=this._minElevation||+Infinity;var dist=this._distance||0;var curr=new L.LatLng(x,y);var prev=data.length?data[data.length-1].latlng:curr;
var delta=curr.distanceTo(prev)*this._distanceFactor;dist=dist+Math.round(delta/1E3*1E5)/1E5;if(typeof z!=="undefined"){z=z*this._heightFactor;eleMax=eleMax<z?z:eleMax;eleMin=eleMin>z?z:eleMin;data.push({dist:dist,x:x,y:y,z:z,latlng:curr})}this._data=data;this._distance=dist;this._maxElevation=eleMax;this._minElevation=eleMin},_addToChartDiv:function(map){this._appendElevationDiv(map._container).appendChild(this.onAdd(map))},_appendChart:function(svg){var g=svg.append("g").attr("transform","translate("+
this.options.margins.left+","+this.options.margins.top+")");this._appendGrid(g);this._appendAreaPath(g);this._appendAxis(g);this._appendFocusRect(g);this._appendMouseFocusG(g)},_appendElevationDiv:function(container){var eleDiv=document.querySelector(this.options.elevationDiv);if(!eleDiv){eleDiv=L.DomUtil.create("div","leaflet-control elevation elevation-div");this.options.elevationDiv="#elevation-div_"+Math.random().toString(36).substr(2,9);eleDiv.id=this.options.elevationDiv.substr(1);container.parentNode.insertBefore(eleDiv,
...
var link=this._button=L.DomUtil.create("a",iconCssClass,container);link.href="#";link.title=this.options.controlButton.title;if(this.options.collapsed){this._collapse();if(this.options.autohide)L.DomEvent.on(container,"mouseover",this._expand,this).on(container,"mouseout",this._collapse,this);else L.DomEvent.on(link,"click",L.DomEvent.stop).on(link,"click",this._toggle,this);L.DomEvent.on(link,"focus",this._toggle,this);this._map.on("click",this._collapse,this)}}else;},_isJSONDoc:function(doc,lazy){lazy=
typeof lazy==="undefined"?true:lazy;if(typeof doc==="string"&&lazy){doc=doc.trim();return doc.indexOf("{")==0||doc.indexOf("[")==0}else{try{JSON.parse(doc.toString())}catch(e){if(typeof doc==="object"&&lazy)return true;console.warn(e);return false}return true}},_isXMLDoc:function(doc,lazy){lazy=typeof lazy==="undefined"?true:lazy;if(typeof doc==="string"&&lazy){doc=doc.trim();return doc.indexOf("<")==0}else{var documentElement=(doc?doc.ownerDocument||doc:0).documentElement;return documentElement?
documentElement.nodeName!=="HTML":false}},_isDomVisible:function(elem){return!!(elem.offsetWidth||elem.offsetHeight||elem.getClientRects().length)},_isVisible:function(elem){if(!elem)return false;var styles=window.getComputedStyle(elem);function isVisibleByStyles(elem,styles){return styles.visibility!=="hidden"&&styles.display!=="none"}function isAboveOtherElements(elem,styles){var boundingRect=elem.getBoundingClientRect();var left=boundingRect.left+1;var right=boundingRect.right-1;var top=boundingRect.top+
1;var bottom=boundingRect.bottom-1;var above=true;var pointerEvents=elem.style.pointerEvents;if(styles["pointer-events"]=="none")elem.style.pointerEvents="auto";if(document.elementFromPoint(left,top)!==elem)above=false;if(document.elementFromPoint(right,top)!==elem)above=false;elem.style.pointerEvents=pointerEvents;return above}if(!isVisibleByStyles(elem,styles))return false;if(!isAboveOtherElements(elem,styles))return false;return true},_lazyLoadJS:function(url,skip){if(typeof skip=="undefined")skip=
false;if(skip instanceof Promise)return skip;return new Promise(function(resolve,reject){if(skip)return resolve();var tag=document.createElement("script");tag.addEventListener("load",resolve,{once:true});tag.src=url;document.head.appendChild(tag)})},_mouseenterHandler:function(){if(this.fire)this.fire("elechart_enter",null,true);if(this._map)this._map.fire("elechart_enter",null,true)},_mousemoveHandler:function(d,i,ctx){if(!this._data||this._data.length===0)return;var coords=d3.mouse(this._focusRect.node());
var xCoord=coords[0];var item=this._data[this._findItemForX(xCoord)];this._hidePositionMarker();this._showDiagramIndicator(item,xCoord);this._showPositionMarker(item);this._setMapView(item);if(this._map&&this._map._container)L.DomUtil.addClass(this._map._container,"elechart-hover");var evt={data:item};if(this.fire){this.fire("elechart_change",evt,true);this.fire("elechart_hover",evt,true)}if(this._map){this._map.fire("elechart_change",evt,true);this._map.fire("elechart_hover",evt,true)}},_mousemoveLayerHandler:function(e){if(!this._data||
this._data.length===0)return;var latlng=e.latlng;var item=this._findItemForLatLng(latlng);if(item){var x=item.xDiagCoord;this._hidePositionMarker();this._showDiagramIndicator(item,x);this._showPositionMarker(item)}},_mouseoutHandler:function(){if(!this.options.detached)this._hidePositionMarker();if(this._map&&this._map._container)L.DomUtil.removeClass(this._map._container,"elechart-hover");if(this.fire)this.fire("elechart_leave",null,true);if(this._map)this._map.fire("elechart_leave",null,true)},
_mousewheelHandler:function(e){if(this._map.gestureHandling&&this._map.gestureHandling._enabled)return;var ll=this._selectedItem?this._selectedItem.latlng:this._map.getCenter();var z=e.deltaY>0?this._map.getZoom()-1:this._map.getZoom()+1;this._resetDrag();this._map.flyTo(ll,z)},_resetDrag:function(){if(this._dragRectangleG){this._dragRectangleG.remove();this._dragRectangleG=null;this._dragRectangle=null;this._hidePositionMarker()}},_resetView:function(){this._resetDrag();this._hidePositionMarker();
if(this._map)this._map.fitBounds(this._fullExtent)},_resizeChart:function(){if(this.options.responsive)if(this.options.detached){var newWidth=this.eleDiv.offsetWidth;if(newWidth<=0)return;this.options.width=newWidth;this.eleDiv.innerHTML="";var container=this.onAdd(this._map);this.eleDiv.appendChild(container)}else{this._map.removeControl(this._container);this.addTo(this._map)}},_showDiagramIndicator:function(item,xCoordinate){var opts=this.options;this._focusG.style("visibility","visible");this._mousefocus.attr("x1",
xCoordinate).attr("y1",0).attr("x2",xCoordinate).attr("y2",this._height()).classed("hidden",false);var alt=item.z,dist=item.dist,ll=item.latlng,numY=opts.hoverNumber.formatter(alt,opts.hoverNumber.decimalsY),numX=opts.hoverNumber.formatter(dist,opts.hoverNumber.decimalsX);this._focuslabeltext.attr("y",this._y(item.z)).style("font-weight","700");this._focuslabelX.text(numX+" "+this._xLabel).attr("x",xCoordinate+10);this._focuslabelY.text(numY+" "+this._yLabel).attr("x",xCoordinate+10);var focuslabeltext=
this._focuslabeltext.node();if(this._isDomVisible(focuslabeltext)){var bbox=focuslabeltext.getBBox();var padding=2;this._focuslabelrect.attr("x",bbox.x-padding).attr("y",bbox.y-padding).attr("width",bbox.width+padding*2).attr("height",bbox.height+padding*2);if(xCoordinate>=this._width()/2){this._focuslabelrect.attr("x",this._focuslabelrect.attr("x")-this._focuslabelrect.attr("width")-padding*2-10);this._focuslabelX.attr("x",this._focuslabelX.attr("x")-this._focuslabelrect.attr("width")-padding*2-
10);this._focuslabelY.attr("x",this._focuslabelY.attr("x")-this._focuslabelrect.attr("width")-padding*2-10)}}},_toggle:function(){if(L.DomUtil.hasClass(this._container,"elevation-expanded"))this._collapse();else this._expand()},_setMapView:function(item){if(!this.options.followMarker||!this._map)return;var zoom=this._map.getZoom();zoom=zoom<this._zFollow?this._zFollow:zoom;this._map.setView(item.latlng,zoom,{animate:true,duration:.25})},_showPositionMarker:function(item){this._selectedItem=item;if(this._map&&
!this._map.getPane("elevationPane")){this._map.createPane("elevationPane");this._map.getPane("elevationPane").style.zIndex=625;this._map.getPane("elevationPane").style.pointerEvents="none"}if(this.options.marker=="elevation-line")this._updatePositionMarker(item);else if(this.options.marker=="position-marker")this._updateLeafletMarker(item)},_updateAxis:function(){this._grid.selectAll("g").remove();this._axis.selectAll("g").remove();this._appendXGrid(this._grid);this._appendYGrid(this._grid);this._appendXaxis(this._axis);
...
normalizedY).text(numY+" "+this._yLabel).style("visibility","visible")},_updateLeafletMarker:function(item){var ll=item.latlng;if(!this._marker){this._marker=new L.Marker(ll,{icon:this.options.MarkerIcon,zIndexOffset:1E6});this._marker.addTo(this._map,{pane:"elevationPane"})}else this._marker.setLatLng(ll)},_updatePointG:function(item){this._pointG.attr("transform","translate("+item.x+","+item.y+")").style("visibility","visible")},_updatePositionMarker:function(item){var point=this._map.latLngToLayerPoint(item.latlng);
var layerpoint={dist:item.dist,x:point.x,y:point.y,z:item.z};if(!this._mouseHeightFocus){L.svg({pane:"elevationPane"}).addTo(this._map);var layerpane=d3.select(this._map.getContainer()).select(".leaflet-elevation-pane svg");this._appendPositionMarker(layerpane)}this._updatePointG(layerpoint);this._updateHeightIndicator(layerpoint)},_updateSummary:function(){if(this.options.summary){this.track_info=this.track_info||{};this.track_info.distance=this._distance||0;this.track_info.elevation_max=this._maxElevation||
0;this.track_info.elevation_min=this._minElevation||0;d3.select(this.summaryDiv).html('<span class="totlen"><span class="summarylabel">Total Length: </span><span class="summaryvalue">'+this.track_info.distance.toFixed(2)+" "+this._xLabel+'</span></span><span class="maxele"><span class="summarylabel">Max Elevation: </span><span class="summaryvalue">'+this.track_info.elevation_max.toFixed(2)+" "+this._yLabel+'</span></span><span class="minele"><span class="summarylabel">Min Elevation: </span><span class="summaryvalue">'+
this.track_info.elevation_min.toFixed(2)+" "+this._yLabel+"</span></span>")}if(this.options.downloadLink&&this._downloadURL){var span=document.createElement("span");span.className="download";var save=document.createElement("a");save.innerHTML="Download";save.href="#";save.onclick=function(e){e.preventDefault();var evt={confirm:this._saveFile.bind(this,this._downloadURL)};var type=this.options.downloadLink;if(type=="modal"){if(typeof CustomEvent==="function")document.dispatchEvent(new CustomEvent("eletrack_download",
{detail:evt}));if(this.fire)this.fire("eletrack_download",evt);if(this._map)this._map.fire("eletrack_download",evt)}else if(type=="link"||type===true)evt.confirm()}.bind(this);this.summaryDiv.appendChild(span).appendChild(save)}},_width:function(){var opts=this.options;return opts.width-opts.margins.left-opts.margins.right}});L.control.elevation=function(options){return new L.Control.Elevation(options)};
