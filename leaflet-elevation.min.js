...
300,this);if(this.options.placeholder)this.options.loadData.lazy=this.options.loadData.defer=true},isVisibile:function(element){function isVisibleByStyles(element){var styles=window.getComputedStyle(element);return styles.visibility!=="hidden"&&styles.display!=="none"}function isBehindOtherElement(element){var boundingRect=element.getBoundingClientRect();var left=boundingRect.left+1;var right=boundingRect.right-1;var top=boundingRect.top+1;var bottom=boundingRect.bottom-1;if(document.elementFromPoint(left,
top)!==element)return true;if(document.elementFromPoint(right,top)!==element)return true;return false}if(!element)return false;if(!isVisibleByStyles(element))return false;if(isBehindOtherElement(element))return false;return true},load:function(data,opts){this.loadData(data,opts)},loadChart:function(map){this.addTo(map)},loadData:function(data,opts){opts=L.extend({},this.options.loadData,opts);if(opts.defer)this.loadDefer(data,opts);else if(opts.lazy)this.loadLazy(data,opts);else if(this._isXMLDoc(data))this.loadGPX(data);
...
this.options.loadData,opts);opts.lazy=false;var ticking=false;var scrollFn=L.bind(function(data){if(!ticking){L.Util.requestAnimFrame(function(){if(this.isVisibile(this.placeholder)){window.removeEventListener("scroll",scrollFn);this.loadData(data,opts);this.once("eledata_loaded",function(){if(this.placeholder)this.placeholder.parentNode.removeChild(this.placeholder)},this)}ticking=false},this);ticking=true}},this,data);window.addEventListener("scroll",scrollFn);scrollFn()},onAdd:function(map){this._map=
...
documentElement.nodeName!=="HTML":false}},_isDomVisible:function(elem){return!!(elem.offsetWidth||elem.offsetHeight||elem.getClientRects().length)},_lazyLoadJS:function(url,skip){if(typeof skip=="undefined")skip=false;if(skip instanceof Promise)return skip;return new Promise(function(resolve,reject){if(skip)return resolve();var tag=document.createElement("script");tag.addEventListener("load",resolve,{once:true});tag.src=url;document.head.appendChild(tag)})},_mousemoveHandler:function(d,i,ctx){if(!this._data||
this._data.length===0)return;var coords=d3.mouse(this._focusRect.node());var xCoord=coords[0];var item=this._data[this._findItemForX(xCoord)];this._hidePositionMarker();this._showDiagramIndicator(item,xCoord);this._showPositionMarker(item);this._setMapView(item);var evt={data:item};if(this.fire)this.fire("elechart_change",evt,true);if(this._map)this._map.fire("elechart_change",evt,true)},_mousemoveLayerHandler:function(e){if(!this._data||this._data.length===0)return;var latlng=e.latlng;var item=this._findItemForLatLng(latlng);
...
this._dragRectangleG=null;this._dragRectangle=null;this._hidePositionMarker()}},_resetView:function(){this._resetDrag();this._hidePositionMarker();if(this._map)this._map.fitBounds(this._fullExtent)},_resizeChart:function(){if(this.options.responsive)if(this.options.detached){var newWidth=this.eleDiv.offsetWidth;if(newWidth<=0)return;this.options.width=newWidth;this.eleDiv.innerHTML="";var container=this.onAdd(this._map);this.eleDiv.appendChild(container)}else{this._map.removeControl(this._container);
