...
loadFile:function(url){this._downloadURL=url;try{var xhr=new XMLHttpRequest;xhr.responseType="text";xhr.open("GET",url);xhr.onload=function(){if(xhr.status!==200)throw"Error "+xhr.status+" while fetching remote file: "+url;else this.loadData(xhr.response)}.bind(this);xhr.send()}catch(e){console.warn(e)}},loadGeoJSON:function(data){if(typeof data==="string")data=JSON.parse(data);this.layer=this.geojson=L.geoJson(data,{style:function(feature){return{color:"#566B13",className:"elevation-polyline "+this.options.theme}}.bind(this),
onEachFeature:function(feature,layer){this.addData(feature,layer);this.track_info=this.track_info||{};this.track_info.type="geojson";this.track_info.name=data.name;this.track_info.distance=this._distance;this.track_info.elevation_max=this._maxElevation;this.track_info.elevation_min=this._minElevation}.bind(this)});if(this._map){this._map.once("layeradd",function(e){this._map.fitBounds(this.layer.getBounds());var evt={data:data,layer:this.layer,name:this.track_info.name,track_info:this.track_info};
if(this.fire)this.fire("eledata_loaded",evt,true);if(this._map)this._map.fire("eledata_loaded",evt,true)},this);this.layer.addTo(this._map)}else console.warn("Undefined elevation map object")},loadGPX:function(data){var isJSLoaded=typeof L.GPX!=="function"&&this.options.lazyLoadJS&&!L.Control.Elevation._gpxLazyLoader;L.Control.Elevation._gpxLazyLoader=this._lazyLoadJS("https://cdnjs.cloudflare.com/ajax/libs/leaflet-gpx/1.4.0/gpx.js",!isJSLoaded);L.Control.Elevation._gpxLazyLoader.then(function(data){this.options.gpxOptions.polyline_options.className+=
"elevation-polyline "+this.options.theme;this.layer=this.gpx=new L.GPX(data,this.options.gpxOptions);this.layer.on("loaded",function(e){if(this._map)this._map.fitBounds(e.target.getBounds())},this);this.layer.on("addpoint",function(e){if(e.point._popup)e.point._popup.options.className="elevation-popup";if(e.point._popup&&e.point._popup._content)e.point.bindTooltip(e.point._popup._content,{direction:"top",sticky:true,opacity:1,className:"elevation-tooltip"}).openTooltip()});this.layer.once("addline",
function(e){this.addData(e.line);this.track_info=this.track_info||{};this.track_info.type="gpx";this.track_info.name=this.layer.get_name();this.track_info.distance=this._distance;this.track_info.elevation_max=this._maxElevation;this.track_info.elevation_min=this._minElevation;var evt={data:data,layer:this.layer,name:this.track_info.name,track_info:this.track_info};if(this.fire)this.fire("eledata_loaded",evt,true);if(this._map)this._map.fire("eledata_loaded",evt,true)},this);if(this._map)this.layer.addTo(this._map);
else console.warn("Undefined elevation map object")}.bind(this,data))},onAdd:function(map){this._map=map;var opts=this.options;var container=this._container=L.DomUtil.create("div","elevation-control elevation");if(!this.options.detached)L.DomUtil.addClass(container,"leaflet-control");if(this.options.theme)L.DomUtil.addClass(container,opts.theme);var isJSLoaded=typeof d3!=="object"&&this.options.lazyLoadJS&&!L.Control.Elevation._d3LazyLoader;L.Control.Elevation._d3LazyLoader=this._lazyLoadJS("https://unpkg.com/d3@4.13.0/build/d3.min.js",
...
eleMin},_addToChartDiv:function(map){this._appendElevationDiv(map._container).appendChild(this.onAdd(map))},_appendChart:function(svg){var g=svg.append("g").attr("transform","translate("+this.options.margins.left+","+this.options.margins.top+")");this._appendGrid(g);this._appendAreaPath(g);this._appendAxis(g);this._appendFocusRect(g);this._appendMouseFocusG(g)},_appendElevationDiv:function(container){var eleDiv=document.querySelector(this.options.elevationDiv);if(!eleDiv){eleDiv=L.DomUtil.create("div",
"leaflet-control elevation elevation-div");this.options.elevationDiv="#elevation-div_"+Math.random().toString(36).substr(2,9);eleDiv.id=this.options.elevationDiv.substr(1);container.parentNode.insertBefore(eleDiv,container.nextSibling)}if(this.options.detached){L.DomUtil.addClass(eleDiv,"elevation-detached");L.DomUtil.removeClass(eleDiv,"leaflet-control")}this.eleDiv=eleDiv;return this.eleDiv},_appendXaxis:function(axis){axis.append("g").attr("class","x axis").attr("transform","translate(0,"+this._height()+
")").call(d3.axisBottom().scale(this._x).ticks(this.options.xTicks)).append("text").attr("x",this._width()+6).attr("y",30).text(this._xLabel)},_appendXGrid:function(grid){grid.append("g").attr("class","x grid").attr("transform","translate(0,"+this._height()+")").call(d3.axisBottom().scale(this._x).ticks(this.options.xTicks).tickSize(-this._height()).tickFormat(""))},_appendYaxis:function(axis){axis.append("g").attr("class","y axis").call(d3.axisLeft().scale(this._y).ticks(this.options.yTicks)).append("text").attr("x",
...
_deepExtend:function(out){out=out||{};for(var i=1,len=arguments.length;i<len;++i){var obj=arguments[i];if(!obj)continue;for(var key in obj){if(!obj.hasOwnProperty(key))continue;if(Object.prototype.toString.call(obj[key])==="[object Object]"){out[key]=this._deepExtend(out[key],obj[key]);continue}out[key]=obj[key]}}return out},_dragHandler:function(){d3.event.preventDefault();d3.event.stopPropagation();this._gotDragged=true;this._drawDragRectangle()},_dragEndHandler:function(){if(!this._dragStartCoords||
!this._dragCurrentCoords||!this._gotDragged){this._dragStartCoords=null;this._gotDragged=false;if(this._draggingEnabled)this._resetDrag();return}var item1=this._findItemForX(this._dragStartCoords[0]),item2=this._findItemForX(this._dragCurrentCoords[0]);if(item1==item2)return;this._hidePositionMarker();this._fitSection(item1,item2);this._dragStartCoords=null;this._gotDragged=false;var evt={data:{dragstart:this._data[item1],dragend:this._data[item2]}};if(this.fire)this.fire("elechart_dragged",evt,true);
if(this._map)this._map.fire("elechart_dragged",evt,true)},_dragStartHandler:function(){d3.event.preventDefault();d3.event.stopPropagation();this._gotDragged=false;this._dragStartCoords=d3.mouse(this._focusRect.node())},_drawDragRectangle:function(){if(!this._dragStartCoords||!this._draggingEnabled)return;var dragEndCoords=this._dragCurrentCoords=d3.mouse(this._focusRect.node());var x1=Math.min(this._dragStartCoords[0],dragEndCoords[0]),x2=Math.max(this._dragStartCoords[0],dragEndCoords[0]);if(!this._dragRectangle&&
!this._dragRectangleG){var g=d3.select(this._container).select("svg").select("g");this._dragRectangleG=g.insert("g",".mouse-focus-group");this._dragRectangle=this._dragRectangleG.append("rect").attr("width",x2-x1).attr("height",this._height()).attr("x",x1).attr("class","mouse-drag").style("pointer-events","none")}else this._dragRectangle.attr("width",x2-x1).attr("x",x1)},_expand:function(){if(this._container){L.DomUtil.removeClass(this._container,"elevation-collapsed");L.DomUtil.addClass(this._container,
...
doc:0).documentElement;return documentElement?documentElement.nodeName!=="HTML":false}},_isDomVisible:function(elem){return!!(elem.offsetWidth||elem.offsetHeight||elem.getClientRects().length)},_lazyLoadJS:function(url,skip){if(typeof skip=="undefined")skip=false;return new Promise(function(resolve,reject){if(skip)resolve();var tag=document.createElement("script");tag.addEventListener("load",resolve,{once:true});tag.src=url;document.head.appendChild(tag)})},_mousemoveHandler:function(d,i,ctx){if(!this._data||
this._data.length===0)return;var coords=d3.mouse(this._focusRect.node());var xCoord=coords[0];var item=this._data[this._findItemForX(xCoord)];this._hidePositionMarker();this._showDiagramIndicator(item,xCoord);this._showPositionMarker(item);if(this.options.followMarker){var zoom=this._map.getZoom();zoom=zoom<this._zFollow?this._zFollow:zoom;this._map.setView(item.latlng,zoom)}var evt={data:item};if(this.fire)this.fire("elechart_change",evt,true);if(this._map)this._map.fire("elechart_change",evt,true)},
_mousemoveLayerHandler:function(e){if(!this._data||this._data.length===0)return;var latlng=e.latlng;var item=this._findItemForLatLng(latlng);if(item){var x=item.xDiagCoord;this._hidePositionMarker();this._showDiagramIndicator(item,x);this._showPositionMarker(item)}},_mouseoutHandler:function(){if(!this.options.detached)this._hidePositionMarker()},_mousewheelHandler:function(e){if(this._map.gestureHandling&&this._map.gestureHandling._enabled)return;var ll=this._selectedItem?this._selectedItem.latlng:
this._map.getCenter();var z=e.deltaY>0?this._map.getZoom()-1:this._map.getZoom()+1;this._resetDrag();this._map.flyTo(ll,z)},_resetDrag:function(){if(this._dragRectangleG){this._dragRectangleG.remove();this._dragRectangleG=null;this._dragRectangle=null;this._hidePositionMarker()}},_resetView:function(){this._resetDrag();this._hidePositionMarker();if(this._map)this._map.fitBounds(this._fullExtent)},_resizeChart:function(){if(this.options.responsive)if(this.options.detached){var newWidth=this.eleDiv.offsetWidth;
...
" "+this._yLabel+'</span></span><span class="minele"><span class="summarylabel">Min Elevation: </span><span class="summaryvalue">'+this.track_info.elevation_min.toFixed(2)+" "+this._yLabel+"</span></span>")}if(this.options.downloadLink&&this._downloadURL){var span=document.createElement("span");span.className="download";var save=document.createElement("a");save.className="download-link";save.innerHTML="Download";save.href="#";save.download="";(function(save,fileURL){save.onclick=function(e){e.preventDefault();
var element=document.createElement("a");element.href=fileURL;element.target="_new";element.download="";element.style.display="none";document.body.appendChild(element);element.click();document.body.removeChild(element)}})(save,this._downloadURL);this.summaryDiv.appendChild(span).appendChild(save)}},_width:function(){var opts=this.options;return opts.width-opts.margins.left-opts.margins.right}});L.control.elevation=function(options){return new L.Control.Elevation(options)};
