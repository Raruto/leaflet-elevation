...
x=y;y=tmp}var data=this._data||[];var eleMax=this._maxElevation||-Infinity;var eleMin=this._minElevation||+Infinity;var dist=this._distance||0;var curr=new L.LatLng(x,y);var prev=data.length?data[data.length-1].latlng:curr;var delta=curr.distanceTo(prev)*this._distanceFactor;dist=dist+Math.round(delta/1E3*1E5)/1E5;if(typeof z!=="undefined"){z=z*this._heightFactor;eleMax=eleMax<z?z:eleMax;eleMin=eleMin>z?z:eleMin;data.push({dist:dist,x:x,y:y,z:z,latlng:curr})}this._data=data;this._distance=dist;this._maxElevation=
eleMax;this._minElevation=eleMin},_addToChartDiv:function(map){this._appendElevationDiv(map._container).appendChild(this.onAdd(map))},_appendChart:function(svg){var g=svg.append("g").attr("transform","translate("+this.options.margins.left+","+this.options.margins.top+")");this._appendGrid(g);this._appendAreaPath(g);this._appendAxis(g);this._appendFocusRect(g);this._appendMouseFocusG(g)},_appendElevationDiv:function(container){var eleDiv=document.querySelector(this.options.elevationDiv);if(!eleDiv){eleDiv=
L.DomUtil.create("div","leaflet-control elevation elevation-div");this.options.elevationDiv="#elevation-div_"+Math.random().toString(36).substr(2,9);eleDiv.id=this.options.elevationDiv.substr(1);container.parentNode.insertBefore(eleDiv,container.nextSibling)}if(this.options.detached){L.DomUtil.addClass(eleDiv,"elevation-detached");L.DomUtil.removeClass(eleDiv,"leaflet-control")}this.eleDiv=eleDiv;return this.eleDiv},_appendXaxis:function(axis){axis.append("g").attr("class","x axis").attr("transform",
