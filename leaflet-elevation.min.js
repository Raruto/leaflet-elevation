...
i,ctx){if(!this._data||this._data.length===0)return;var coords=d3.mouse(this._focusRect.node());var xCoord=coords[0];var item=this._data[this._findItemForX(xCoord)];this._hidePositionMarker();this._showDiagramIndicator(item,xCoord);this._showPositionMarker(item);if(this.options.followMarkerLocation){var zoom=this._map.getZoom();zoom=zoom<this._zFollow?this._zFollow:zoom;this._map.setView(item.latlng,zoom)}this._map.fireEvent("elechart_change",{data:item},true)},_mousemoveLayerHandler:function(e){if(!this._data||
