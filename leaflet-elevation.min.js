...
300,this);if(this.options.placeholder)this.options.loadData.lazy=this.options.loadData.defer=true},isVisibile:function(element){if(!element)return false;var styles=window.getComputedStyle(element);function isVisibleByStyles(element,styles){return styles.visibility!=="hidden"&&styles.display!=="none"}function isAboveOtherElements(element,styles){var boundingRect=element.getBoundingClientRect();var left=boundingRect.left+1;var right=boundingRect.right-1;var top=boundingRect.top+1;var bottom=boundingRect.bottom-
1;var above=true;var pointerEvents=element.style.pointerEvents;if(styles["pointer-events"]=="none")element.style.pointerEvents="auto";if(document.elementFromPoint(left,top)!==element)above=false;if(document.elementFromPoint(right,top)!==element)above=false;if(pointerEvents)element.style.pointerEvents=pointerEvents;return above}if(!isVisibleByStyles(element,styles))return false;if(!isAboveOtherElements(element,styles))return false;return true},load:function(data,opts){this.loadData(data,opts)},loadChart:function(map){this.addTo(map)},
...
this._lazyLoadJS("https://cdnjs.cloudflare.com/ajax/libs/leaflet-gpx/1.4.0/gpx.js",L.Control.Elevation._gpxLazyLoader);L.Control.Elevation._gpxLazyLoader.then(callback)}else callback.call()},loadLazy:function(data,opts){opts=L.extend({},this.options.loadData,opts);opts.lazy=false;var ticking=false;var scrollFn=L.bind(function(data){if(!ticking){L.Util.requestAnimFrame(function(){if(this.isVisibile(this.placeholder)){window.removeEventListener("scroll",scrollFn);this.loadData(data,opts);this.once("eledata_loaded",
function(){if(this.placeholder)this.placeholder.parentNode.removeChild(this.placeholder)},this)}ticking=false},this);ticking=true}},this,data);window.addEventListener("scroll",scrollFn);scrollFn()},onAdd:function(map){this._map=map;var container=this._container=L.DomUtil.create("div","elevation-control elevation");if(!this.options.detached)L.DomUtil.addClass(container,"leaflet-control");if(this.options.theme)L.DomUtil.addClass(container,this.options.theme);if(this.options.placeholder&&!this._data){this.placeholder=
L.DomUtil.create("img","elevation-placeholder");if(typeof this.options.placeholder==="string"){this.placeholder.src=this.options.placeholder;this.placeholder.alt=""}else for(var i in this.options.placeholder)this.placeholder.setAttribute(i,this.options.placeholder[i]);container.insertBefore(this.placeholder,container.firstChild)}var callback=function(map,container){this._initToggle(container);this._initChart(container);this._applyData();this._map.on("zoom viewreset zoomanim",this._hidePositionMarker,
