...
for(var id in this._layers){L.DomUtil.removeClass(this._layers[id]._path,"elevation-polyline");L.DomUtil.removeClass(this._layers[id]._path,this.options.theme)}},_collapse:function(){if(this._container){L.DomUtil.removeClass(this._container,"elevation-expanded");L.DomUtil.addClass(this._container,"elevation-collapsed")}},_deepMerge:function(target,sources){var $jscomp$restParams=[];for(var $jscomp$restIndex=1;$jscomp$restIndex<arguments.length;++$jscomp$restIndex)$jscomp$restParams[$jscomp$restIndex-
1]=arguments[$jscomp$restIndex];{var sources$2=$jscomp$restParams;if(!sources$2.length)return target;var source=sources$2.shift();if(this._isObject(target)&&this._isObject(source))for(var key in source)if(this._isObject(source[key])){if(!target[key]){var $jscomp$compprop0={};Object.assign(target,($jscomp$compprop0[key]={},$jscomp$compprop0))}this._deepMerge(target[key],source[key])}else{var $jscomp$compprop1={};Object.assign(target,($jscomp$compprop1[key]=source[key],$jscomp$compprop1))}return this._deepMerge.apply(this,
[].concat([target],$jscomp.arrayFromIterable(sources$2)))}},_saveFile:function(fileUrl){var d=document,a=d.createElement("a"),b=d.body;a.href=fileUrl;a.target="_new";a.download="";a.style.display="none";b.appendChild(a);a.click();b.removeChild(a)},_dragHandler:function(){d3.event.preventDefault();d3.event.stopPropagation();this._gotDragged=true;this._drawDragRectangle()},_dragEndHandler:function(){if(!this._dragStartCoords||!this._dragCurrentCoords||!this._gotDragged){this._dragStartCoords=null;this._gotDragged=
false;if(this._draggingEnabled)this._resetDrag();if(this._chartEnabled){this._clearChart();this._clearPath();this._chartEnabled=false}else{this._resizeChart();this._chartEnabled=true}return}var item1=this._findItemForX(this._dragStartCoords[0]),item2=this._findItemForX(this._dragCurrentCoords[0]);if(item1==item2)return;this._hidePositionMarker();this._fitSection(item1,item2);this._dragStartCoords=null;this._gotDragged=false;var evt={data:{dragstart:this._data[item1],dragend:this._data[item2]}};if(this.fire)this.fire("elechart_dragged",
evt,true);if(this._map)this._map.fire("elechart_dragged",evt,true)},_dragStartHandler:function(){d3.event.preventDefault();d3.event.stopPropagation();this._gotDragged=false;this._dragStartCoords=d3.mouse(this._focusRect.node())},_drawDragRectangle:function(){if(!this._dragStartCoords||!this._draggingEnabled)return;var dragEndCoords=this._dragCurrentCoords=d3.mouse(this._focusRect.node());var x1=Math.min(this._dragStartCoords[0],dragEndCoords[0]),x2=Math.max(this._dragStartCoords[0],dragEndCoords[0]);
...
this)}}else;},_isObject:function(item){return item&&typeof item==="object"&&!Array.isArray(item)},_isJSONDoc:function(doc,lazy){lazy=typeof lazy==="undefined"?true:lazy;if(typeof doc==="string"&&lazy){doc=doc.trim();return doc.indexOf("{")==0||doc.indexOf("[")==0}else{try{JSON.parse(doc.toString())}catch(e){if(typeof doc==="object"&&lazy)return true;console.warn(e);return false}return true}},_isXMLDoc:function(doc,lazy){lazy=typeof lazy==="undefined"?true:lazy;if(typeof doc==="string"&&lazy){doc=
doc.trim();return doc.indexOf("<")==0}else{var documentElement=(doc?doc.ownerDocument||doc:0).documentElement;return documentElement?documentElement.nodeName!=="HTML":false}},_isDomVisible:function(elem){return!!(elem.offsetWidth||elem.offsetHeight||elem.getClientRects().length)},_isVisible:function(elem){if(!elem)return false;var styles=window.getComputedStyle(elem);function isVisibleByStyles(elem,styles){return styles.visibility!=="hidden"&&styles.display!=="none"}function isAboveOtherElements(elem,
styles){var boundingRect=elem.getBoundingClientRect();var left=boundingRect.left+1;var right=boundingRect.right-1;var top=boundingRect.top+1;var bottom=boundingRect.bottom-1;var above=true;var pointerEvents=elem.style.pointerEvents;if(styles["pointer-events"]=="none")elem.style.pointerEvents="auto";if(document.elementFromPoint(left,top)!==elem)above=false;if(document.elementFromPoint(right,top)!==elem)above=false;elem.style.pointerEvents=pointerEvents;return above}if(!isVisibleByStyles(elem,styles))return false;
if(!isAboveOtherElements(elem,styles))return false;return true},_lazyLoadJS:function(url,skip){if(typeof skip=="undefined")skip=false;if(skip instanceof Promise)return skip;return new Promise(function(resolve,reject){if(skip)return resolve();var tag=document.createElement("script");tag.addEventListener("load",resolve,{once:true});tag.src=url;document.head.appendChild(tag)})},_mouseenterHandler:function(){if(this.fire)this.fire("elechart_enter",null,true);if(this._map)this._map.fire("elechart_enter",
null,true)},_mousemoveHandler:function(d,i,ctx){if(!this._data||this._data.length===0||!this._chartEnabled)return;var coords=d3.mouse(this._focusRect.node());var xCoord=coords[0];var item=this._data[this._findItemForX(xCoord)];this._hidePositionMarker();this._showDiagramIndicator(item,xCoord);this._showPositionMarker(item);this._setMapView(item);if(this._map&&this._map._container)L.DomUtil.addClass(this._map._container,"elechart-hover");var evt={data:item};if(this.fire){this.fire("elechart_change",
evt,true);this.fire("elechart_hover",evt,true)}if(this._map){this._map.fire("elechart_change",evt,true);this._map.fire("elechart_hover",evt,true)}},_mousemoveLayerHandler:function(e){if(!this._data||this._data.length===0)return;var latlng=e.latlng;var item=this._findItemForLatLng(latlng);if(item){var xCoord=item.xDiagCoord;this._hidePositionMarker();this._showDiagramIndicator(item,xCoord);this._showPositionMarker(item)}},_mouseoutHandler:function(){if(!this.options.detached)this._hidePositionMarker();
if(this._map&&this._map._container)L.DomUtil.removeClass(this._map._container,"elechart-hover");if(this.fire)this.fire("elechart_leave",null,true);if(this._map)this._map.fire("elechart_leave",null,true)},_mousewheelHandler:function(e){if(this._map.gestureHandling&&this._map.gestureHandling._enabled)return;var ll=this._selectedItem?this._selectedItem.latlng:this._map.getCenter();var z=e.deltaY>0?this._map.getZoom()-1:this._map.getZoom()+1;this._resetDrag();this._map.flyTo(ll,z)},_resetDrag:function(){if(this._dragRectangleG){this._dragRectangleG.remove();
this._dragRectangleG=null;this._dragRectangle=null;this._hidePositionMarker()}},_resetView:function(){if(this._map&&this._map._isFullscreen)return;this._resetDrag();this._hidePositionMarker();if(this._map)this._map.fitBounds(this._fullExtent)},_resizeChart:function(){if(this.options.responsive)if(this.options.detached){var newWidth=this.eleDiv.offsetWidth;if(newWidth<=0)return;this.options.width=newWidth;this.eleDiv.innerHTML="";this.eleDiv.appendChild(this.onAdd(this._map))}else{this._map.removeControl(this._container);
this.addTo(this._map)}},_showDiagramIndicator:function(item,xCoordinate){if(!this._chartEnabled)return;var opts=this.options;this._focusG.style("visibility","visible");this._mousefocus.attr("x1",xCoordinate).attr("y1",0).attr("x2",xCoordinate).attr("y2",this._height()).classed("hidden",false);var alt=item.z,dist=item.dist,ll=item.latlng,numY=opts.hoverNumber.formatter(alt,opts.hoverNumber.decimalsY),numX=opts.hoverNumber.formatter(dist,opts.hoverNumber.decimalsX);this._focuslabeltext.attr("y",this._y(item.z)).style("font-weight",
