...
_updateLeafletMarker:function(item){var ll=item.latlng;if(!this._marker){this._marker=new L.Marker(ll,{icon:this.options.MarkerIcon,zIndexOffset:1E6});this._marker.addTo(this._map,{pane:"elevationPane"})}else this._marker.setLatLng(ll)},_updatePointG:function(item){this._pointG.attr("transform","translate("+item.x+","+item.y+")").style("visibility","visible")},_updatePositionMarker:function(item){var point=this._map.latLngToLayerPoint(item.latlng);var layerpoint={dist:item.dist,x:point.x,y:point.y,
z:item.z};if(!this._mouseHeightFocus){L.svg({pane:"elevationPane"}).addTo(this._map);var layerpane=d3.select(this._map.getContainer()).select(".leaflet-elevation-pane svg");this._appendPositionMarker(layerpane)}this._updatePointG(layerpoint);this._updateHeightIndicator(layerpoint)},_updateSummary:function(){if(this.options.summary){this.track_info=this.track_info||{};this.track_info.distance=this._distance||0;this.track_info.elevation_max=this._maxElevation||0;this.track_info.elevation_min=this._minElevation||
0;d3.select(this.summaryDiv).html('<span class="totlen"><span class="summarylabel">Total Length: </span><span class="summaryvalue">'+this.track_info.distance.toFixed(2)+" "+this._xLabel+'</span></span><span class="maxele"><span class="summarylabel">Max Elevation: </span><span class="summaryvalue">'+this.track_info.elevation_max.toFixed(2)+" "+this._yLabel+'</span></span><span class="minele"><span class="summarylabel">Min Elevation: </span><span class="summaryvalue">'+this.track_info.elevation_min.toFixed(2)+
" "+this._yLabel+"</span></span>")}if(this.options.downloadLink&&this._downloadURL){var span=document.createElement("span");span.className="download";var save=document.createElement("a");save.innerHTML="Download";save.href="#";save.onclick=function(e){e.preventDefault();var evt={confirm:this._saveFile.bind(this,this._downloadURL)};var type=this.options.downloadLink;if(type=="modal"){if(typeof CustomEvent==="function")document.dispatchEvent(new CustomEvent("eletrack_download",{detail:evt}));if(this.fire)this.fire("eletrack_download",
