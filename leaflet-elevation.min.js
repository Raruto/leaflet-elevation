...
name:this.track_info.name,track_info:this.track_info};if(this.fire)this.fire("eledata_loaded",evt,true);if(this._map)this._map.fire("eledata_loaded",evt,true)},this);this.layer.addTo(this._map)}else console.warn("Undefined elevation map object")},loadGPX:function(data){var skipJSLoading=typeof L.GPX==="function"||this.options.lazyLoadJS&&!L.Control.Elevation._gpxLazyLoader;var callback=function(data){this.options.gpxOptions.polyline_options.className+="elevation-polyline "+this.options.theme;this.layer=
this.gpx=new L.GPX(data,this.options.gpxOptions);this.layer.on("loaded",function(e){if(this._map)this._map.fitBounds(e.target.getBounds())},this);this.layer.on("addpoint",function(e){if(e.point._popup){e.point._popup.options.className="elevation-popup";e.point._popup._content=decodeURI(e.point._popup._content)}if(e.point._popup&&e.point._popup._content)e.point.bindTooltip(e.point._popup._content,{direction:"top",sticky:true,opacity:1,className:"elevation-tooltip"}).openTooltip()});this.layer.once("addline",
...
else console.warn("Undefined elevation map object")}.bind(this,data);if(!skipJSLoading){L.Control.Elevation._gpxLazyLoader=this._lazyLoadJS("https://cdnjs.cloudflare.com/ajax/libs/leaflet-gpx/1.4.0/gpx.js",skipJSLoading);L.Control.Elevation._gpxLazyLoader.then(callback)}else callback.call()},onAdd:function(map){this._map=map;var opts=this.options;var container=this._container=L.DomUtil.create("div","elevation-control elevation");if(!this.options.detached)L.DomUtil.addClass(container,"leaflet-control");
if(this.options.theme)L.DomUtil.addClass(container,opts.theme);var skipJSLoading=typeof d3==="object"||this.options.lazyLoadJS&&!L.Control.Elevation._d3LazyLoader;var callback=function(map,container){this._initToggle(container);this._initChart(container);this._applyData();this._map.on("zoom viewreset zoomanim",this._hidePositionMarker,this);this._map.on("resize",this._resetView,this);this._map.on("resize",this._resizeChart,this);this._map.on("mousedown",this._resetDrag,this);this._map.on("eledata_loaded",
this._updateSummary,this);L.DomEvent.on(this._map._container,"mousewheel",this._resetDrag,this);L.DomEvent.on(this._map._container,"touchstart",this._resetDrag,this)}.bind(this,map,container);if(!skipJSLoading){L.Control.Elevation._d3LazyLoader=this._lazyLoadJS("https://unpkg.com/d3@4.13.0/build/d3.min.js",skipJSLoading);L.Control.Elevation._d3LazyLoader.then(callback)}else callback.call();return container},onRemove:function(map){this._container=null},setZFollow:function(zoom){this._zFollow=zoom},
...
documentElement.nodeName!=="HTML":false}},_isDomVisible:function(elem){return!!(elem.offsetWidth||elem.offsetHeight||elem.getClientRects().length)},_lazyLoadJS:function(url,skip){if(typeof skip=="undefined")skip=false;return new Promise(function(resolve,reject){if(skip)return resolve();var tag=document.createElement("script");tag.addEventListener("load",resolve,{once:true});tag.src=url;document.head.appendChild(tag)})},_mousemoveHandler:function(d,i,ctx){if(!this._data||this._data.length===0)return;
var coords=d3.mouse(this._focusRect.node());var xCoord=coords[0];var item=this._data[this._findItemForX(xCoord)];this._hidePositionMarker();this._showDiagramIndicator(item,xCoord);this._showPositionMarker(item);this._setMapView(item);var evt={data:item};if(this.fire)this.fire("elechart_change",evt,true);if(this._map)this._map.fire("elechart_change",evt,true)},_mousemoveLayerHandler:function(e){if(!this._data||this._data.length===0)return;var latlng=e.latlng;var item=this._findItemForLatLng(latlng);
