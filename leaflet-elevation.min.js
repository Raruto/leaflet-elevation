...
true;this._drawDragRectangle()},_dragEndHandler:function(){if(!this._dragStartCoords||!this._dragCurrentCoords||!this._gotDragged){this._dragStartCoords=null;this._gotDragged=false;if(this._draggingEnabled)this._resetDrag();return}var item1=this._findItemForX(this._dragStartCoords[0]),item2=this._findItemForX(this._dragCurrentCoords[0]);if(item1==item2)return;this._hidePositionMarker();this._fitSection(item1,item2);this._dragStartCoords=null;this._gotDragged=false;this._map.fireEvent("elechart_dragged",
...
documentElement.nodeName!=="HTML":false}},_isDomVisible:function(elem){return!!(elem.offsetWidth||elem.offsetHeight||elem.getClientRects().length)},_lazyLoadJS:function(url,skip){if(typeof skip=="undefined")skip=false;return new Promise(function(resolve,reject){if(skip)resolve();var tag=document.createElement("script");tag.addEventListener("load",resolve,{once:true});tag.src=url;document.head.appendChild(tag)})},_mousemoveHandler:function(d,i,ctx){if(!this._data||this._data.length===0)return;var coords=
...
this._yLabel).style("visibility","visible")},_updateLeafletMarker:function(item){var ll=item.latlng;if(!this._marker){this._marker=new L.Marker(ll,{icon:this.options.leafletMarkerIcon,zIndexOffset:1E6});this._marker.addTo(this._map,{pane:"elevationPane"})}else this._marker.setLatLng(ll)},_updatePointG:function(item){this._pointG.attr("transform","translate("+item.x+","+item.y+")").style("visibility","visible")},_updatePositionMarker:function(item){var point=this._map.latLngToLayerPoint(item.latlng);
...
this._distance||0;this.track_info.elevation_max=this._maxElevation||0;this.track_info.elevation_min=this._minElevation||0;d3.select(this.summaryDiv).html('<span class="totlen"><span class="summarylabel">Total Length: </span><span class="summaryvalue">'+this.track_info.distance.toFixed(2)+" "+this._xLabel+'</span></span><span class="maxele"><span class="summarylabel">Max Elevation: </span><span class="summaryvalue">'+this.track_info.elevation_max.toFixed(2)+" "+this._yLabel+'</span></span><span class="minele"><span class="summarylabel">Min Elevation: </span><span class="summaryvalue">'+
this.track_info.elevation_min.toFixed(2)+" "+this._yLabel+"</span></span>")}if(this.options.downloadLink){var span=document.createElement("span");span.className="download";var save=document.createElement("a");save.className="download-link";save.innerHTML="Download";save.href="#";(function(save,fileURL){save.onclick=function(e){e.preventDefault();var element=document.createElement("a");element.href=fileURL;element.target="_new";element.download="";element.style.display="none";document.body.appendChild(element);
element.click();document.body.removeChild(element)}})(save,this._downloadURL);this.summaryDiv.appendChild(span).appendChild(save)}},_width:function(){var opts=this.options;return opts.width-opts.margins.left-opts.margins.right}});L.control.elevation=function(options){return new L.Control.Elevation(options)};
